...
| # | 紀錄 | commit | GitHub Actions run | 結果 |
|---|---|---|---|---|
...
### 3.1 成功紀錄 ✅
基線版本，所有 job 綠燈：test / build / dependency-scan / secret-scan / sast。

![成功紀錄](docs/01-success.png)

### 3.2 故意失敗紀錄 ❌（功能測試）
把 `app/auth.py` 的 `verify_password` 故意改成永遠回傳 `True`，使測試 `test_wrong_password_rejected` 失敗。
**test job 紅燈**，而且 **build job 因 `needs: test` 被 skip**——pipeline 在打包前就擋下缺陷。

![故意失敗：測試紅燈](docs/02-test-fail.png)

### 3.3 修正後再次成功 ✅
還原 `verify_password` 的正確比對邏輯後，test job 恢復綠燈、整條 pipeline 回綠。

![修正後回綠](docs/03-test-fix.png)

### 3.4 套件弱點 — 修補前 / 後
**修補前**：把 `requirements.txt` 的 `PyJWT` 降回 `2.10.1`，該版本帶 7 個 CVE
（PYSEC-2026-176/179/175/177/178/120、PYSEC-2025-183）。**dependency-scan job 紅燈**，
而 test / build 仍綠——凸顯依賴掃描階段獨立把關。

![弱點修補前：依賴掃描紅燈](docs/04-vuln-before.png)

**修補後**：升級為 `PyJWT==2.13.0`（只改版號、程式碼零改動）。pip-audit 乾淨、dependency-scan 回綠。
